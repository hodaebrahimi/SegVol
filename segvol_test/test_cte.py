...
for item in tqdm(test_loader):
    ...
    # Resume-safe: skip cases whose 3 masks already exist
    if all(os.path.exists(join(case_save_dir, f'{n}.nii.gz'))
           for n in category_reflect.values()):
        print(f'Skipping (already done): {case_num}')
        continue
